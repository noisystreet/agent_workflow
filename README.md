...
- **减少隐式状态**：优先显式传递依赖与状态，降低代码的心智负担与跨函数推理难度——  
  - 避免全局/模块级可变状态（包括 Rust `static mut`、Go `var` 全局变量、JS 模块顶层 let、Python 模块级 mutable 对象），改为显式参数注入或上下文传递。  
  - 函数应纯化：输入决定输出，避免隐式依赖外部状态（文件系统、环境变量、单例等）；确需副作用的函数应通过命名和类型系统显式标注（如 Rust `Result` / `impl FnMut`、Go 命名 `func XxxWithSideEffect`、JS `async` 函数等）。  
  - 避免函数体内修改外部传入的可变对象（除非函数名明确表示写入意图，如 `append`、`populate`），建议使用不可变数据结构或在最小范围内隔离副作用。  
  - 上下文对象归约：若多个函数共享同一批状态（如配置、数据库连接），使用显式上下文结构体（如 Rust `Context`、Go `context.Context`、Python `dataclass`）集中传递，而非分别从不同全局源获取。  
...
- **数据校验与防御性编程**：减少因不可信输入或内部契约违反导致的运行时事故——
  - 校验边界统一：所有外部输入在边界层（API 入口、CLI 参数、反序列化时）一次校验，内部函数假设输入已通过校验，避免各层重复且可能不一致的检查。
  - 错误输入策略：明确 fail-fast vs 宽松纠错——开发/测试环境 fail-fast 及早暴露缺陷；生产环境对非关键输入可纠错（如截断超长字段），但需日志记录。
  - 敏感数据脱敏：日志、错误消息、导出数据中涉及的密码、Token、PII 必须在输出前脱敏（如 `mask("abc123") → "ab****"`），可约定统一脱敏函数。
  - 内部不变式断言：不可恢复的内部逻辑违反使用断言（Rust `debug_assert!`、Go `panic("unreachable")`、Python `assert`），不依赖外部输入；与错误处理范式互补。
- **并发与同步策略**：降低并发事故（数据竞争、死锁、资源饥饿）的概率——
  - 共享状态同步：锁粒度优先粗锁（避免死锁），仅在性能热点使用细锁或无锁结构。锁获取顺序需约定并文档化，防止循环等待。
  - async/sync 边界：阻塞操作（IO、锁等待）禁止在 async 上下文中直接调用，须通过 `spawn_blocking` / 线程池转移。`Send + Sync` trait 约束连同 unsafe 理由需文档化。
  - 避免隐式并发：全局可变状态、`lazy_static` / `OnceCell` 初始化的闭包内不应启动后台任务或持有跨协程/线程的可变引用。
  - 上下文传递：分布式场景下 trace_id / span_id / request_id 通过显式上下文参数传递，而非线程局部存储或全局变量，确保跨协程/线程的正确传播。
...
- **CI 流水线**（GitHub Actions / GitLab CI 等）：至少包含 lint（格式检查）、类型检查 / 静态分析、构建、测试。静态分析建议开启警告即失败。测试阶段应执行[测试策略](#5-测试策略)定义的全部测试层级（单元 → 集成 → E2E），并将覆盖率报告上传或归档。  
  **代码质量门控**（由 linter 配置驱动，无需额外脚本）：  
  - **文件行数上限**：如 Rust `clippy::too_many_lines`、Python `pylint --max-module-lines`、JavaScript/Go 等同类规则。  
  - **函数参数上限**：如 Rust `clippy::too_many_arguments`、JavaScript `max-params`、Python `pylint --max-args`。  
  - **函数复杂度上限**：圈复杂度（如 Rust `clippy::cognitive_complexity`、Python `radon cc`、JS `complexity` 规则、Go `gocyclo`）。  
  - **覆盖率门控**：由[测试策略](#5-测试策略)定义阈值，CI 中执行覆盖率检查并决定是否阻断；因其计算开销大，不进入 pre-commit。  
...
- **代码评审规范**：提升评审质量与效率，降低 Agent 与人工审阅者的认知落差——
  - 单 PR 规模上限：建议 ≤ 400 行有效改动（不含测试、配置文件、自动生成代码），超出时要求拆分或附加说明。
  - 必过检查项：代码风格与 lint 通过、测试覆盖变更路径、CHANGELOG 和文档同步更新、无敏感信息泄漏。
  - 自评义务：作者（含 Agent）在 PR 描述中提供变更摘要、测试策略、影响范围；Agent 生成的代码需标注（如 `// AI-generated`），便于审阅者重点检查。
  - 审阅者义务：48h 内首次回复、聚焦逻辑正确性与安全而非格式（格式由 CI 和 pre-commit 保证）、对非阻塞建议标注 `nit:` 前缀。
  - AI 生成代码的特殊要求：必须绑定具体 Issue 或任务描述；Agent 生成但无法自行验证的部分（如安全审计、性能影响）须在 PR 描述中标记，由人工审阅者补充验证。
...
- **性能与资源约束**：在持续集成中建立性能基线，防止非功能特性退步——
  - 热路径标注：核心/高频调用路径应在注释中标注 `HOT_PATH` 并附加基准测试引用，Agent 修改时须关注性能影响。
  - 资源上限约定：连接池、缓存、并发 goroutine/task、消息队列等有显式上限，CI 中以静态分析或测试断言检查是否存在无界容器/增长。
  - 分配敏感：高频路径（每请求、每事件）应避免不必要的堆分配、反射、闭包逃逸等；语言专项规则（如 Rust 减少 `clone()`、Go `-gcflags=-m` 检查逃逸、JS 避免 hot loop 中创建函数）。
  - 复杂度基线：关键算法在注释中标注时间/空间复杂度，破坏性变更（如 O(n) → O(n²)）须在 PR 描述中显式说明理由。
- **技术债务管理**：主动记录与追踪临时方案，防止 Agent 累积不可维护的绕过——
  - TODO 标注规范：`TODO(#issue): 原因` 或 `FIXME(owner): 原因`，必须关联 Issue 或具体责任人，不得出现无上下文 TODO。
  - 技术债务追踪：在 `docs/TECH_DEBT.md` 或项目看板中集中记录已知临时方案、HACK、未完成的迁移，定期评审清理。
  - 弃用流程：弃用函数/API 须跨越两个版本（当前版本标记 `#[deprecated]` / `@deprecated` + JSDoc，下一版本移除），并在 CHANGELOG 中通知。
- **调试与诊断支持**：降低运行时问题定位成本——
  - 断言策略：断言仅用于不可恢复的内部不变式违反（如 switch 穷举失败、数据结构不变量），不得用于输入校验，与防御性编程的错误处理互补。
  - Debug 输出：公开类型应实现 `Debug` / `Display` / `__repr__` / `String()` 等便于诊断的格式化接口，日志中涉及的类型默认使用结构化而非 `ToString`。
  - 可诊断日志：关键决策点（认证决策、路由匹配、状态切换）必须输出 DEBUG 级结构化日志，包含足以复现上下文的键值对，避免「看到怪现象但无法追溯」。
...
- [ ] 编码约定（错误处理、日志、配置管理、依赖策略、数据校验、并发）  
- [ ] CI + pre-commit（含 commit-msg 若需要）  
- [ ] 测试框架选型 + 分层约定 + 覆盖率门控  
- [ ] 软件工程与代码质量门限（第 12 节：静态分析、性能基线、技术债务、调试诊断、死代码/依赖卫生、契约与文档质量等，按项目裁剪）  
- [ ] 代码托管与协作治理（第 8 节：分支保护、代码评审规范、必过检查、PR/Issue 模板、可选 CoC、依赖机器人）  
